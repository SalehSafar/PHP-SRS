...
Public Class Form5
    ...
    Private Sub editingComboBox_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'grey out if qty less than 2
        If (i.Qty <= 2) Then
            OrderGrid.Rows(OrderGrid.CurrentCell.RowIndex).Cells(2).Style.BackColor = Color.Red
        End If
    ...
    Private Sub GetAutoCompleteData(data)
        OpenExcelFile()
    ...
    Private Sub SellBtn_Click(sender As Object, e As EventArgs) Handles SellBtn.Click
        ...
        Dim days As String
        days = day.ToString("dd/MM/yyyy")
        Dim x As Integer
        x = 0
        Dim reportdays As Boolean

        reportdays = False

        Dim blLastRow As Integer
        ...
        With Form1.worksheet
            ...
            blLastRow = Form1.worksheet.Cells(Form1.worksheet.Rows.Count, "H").End(Excel.XlDirection.xlUp).Row
            'shift from an extra row if list has header
        End With

        Dim testing As String
        testing = Form1.worksheet.Cells(blLastRow, 8).value



        If Form1.worksheet.Cells(blLastRow, 8).value.Equals(days) Then
            reportdays = True
        Else
            reportdays = False
        End If
        ...
        For Each row As DataGridViewRow In OrderGrid.Rows
            ...
            If (name = "") Then
                Exit For
            End If
            Dim item = SearchItem(name)

            If (item.Qty < qty) Then
                MessageBox.Show(name + " only left " + qty.ToString() + " so can't sell. Please enter different quantity")
                Return
            End If
        Next
        ...
        For Each row As DataGridViewRow In OrderGrid.Rows
            ...
            Dim updatestock2 = String.Format("Insert into [sheet3$] ([item],[quantity],[price],[description],[total]) values(" + "'" + name + "'" + "," + "'" + qty + "'" + "," + "'" + pric.ToString + "'" + "," + "'" + days + "'" + "," + "'" + (qty * pric).ToString + "'" + ")")

            If reportdays = True Then

                Dim reporttoday = String.Format("Update [sheet3$]" +
                " SET totalsales = totalsales + " + (pric * qty).ToString +
                " WHERE date = " + "'" + days + "'")

                Dim reporttodays As OleDbCommand = New OleDbCommand(reporttoday, _myConnection)

                Dim result3 = reporttodays.ExecuteNonQuery()

            Else

                Dim reportnewday = String.Format("Insert into [sheet3$] ([date],[totalsales]) values(" + "'" + days + "'" + "," + "'" + (qty * pric).ToString + "'" + ")")

                Dim reportnewdays As OleDbCommand = New OleDbCommand(reportnewday, _myConnection)

                Dim result4 = reportnewdays.ExecuteNonQuery()
            ...
            Dim commands2 As OleDbCommand = New OleDbCommand(updatestock2, _myConnection)

            'run command
            Dim result = command.ExecuteNonQuery()

            Dim results2 = commands2.ExecuteNonQuery()
        ...
        Dim alLastRow As Integer


        With Form1.worksheet
            'find the last row of the list
            alLastRow = Form1.worksheet.Cells(Form1.worksheet.Rows.Count, "A").End(Excel.XlDirection.xlUp).Row
            'shift from an extra row if list has header
        End With
        ...
        With Form1.worksheet
            'find the last row of the list
            datelLastRow = Form1.worksheet.Cells(Form1.worksheet.Rows.Count, "H").End(Excel.XlDirection.xlUp).Row
            'shift from an extra row if list has header
        End With
